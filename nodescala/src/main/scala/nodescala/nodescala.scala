...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val loopingSubscription = Future.run()(ct =>
      async {
        while(!ct.isCancelled) {
          val requestAndExg = await { listener.nextRequest() }
          respond(requestAndExg._2, ct, handler(requestAndExg._1))
        }
      } 
    )
    Subscription(listenerSubscription, loopingSubscription)
  }
